...
namespace ModbusTCP
{
    ...
    public class Master
    {
        ...
        private const byte fctReportSlaveID = 17;
        ...
        // ------------------------------------------------------------------------
        /// <summary>Report slave ID synchronous.</summary>
        /// <param name="id">Unique id that marks the transaction. In asynchonous mode this id is given to the callback function.</param>
        /// <param name="unit">Unit identifier (previously slave address). In asynchonous mode this unit is given to the callback function.</param>
        /// <param name="values">Contains the result of function.</param>
        public void ReportSlaveID(ushort id, byte unit, ref byte[] values)
        {
            values = WriteSyncData(CreateReportSlaveIDHeader(id, unit, fctReportSlaveID), id);
        }
        ...
        private byte[] CreateReadHeader(ushort id, byte unit, ushort startAddress, ushort length, byte function)
        {
            ...
            return data;
                ...
                // ------------------------------------------------------------------------
        // Create modbus header for report slave ID action
        private byte[] CreateReportSlaveIDHeader(ushort id, byte unit, byte function)
        {
        ...
        private byte[] WriteSyncData(byte[] write_data, ushort id)
        {
            ...
            if (tcpSynCl.Connected)
            {
                try
                {
                    ...
                    // ------------------------------------------------------------
                    // Write response data
                    else if ((function >= fctWriteSingleCoil) && (function != fctReadWriteMultipleRegister) && (function != fctReportSlaveID))
                    {
                        data = new byte[2];
